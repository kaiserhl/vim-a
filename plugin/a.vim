...
call <SID>AddAlternateExtensionMapping('.h',".cc,.c,.cpp,.cxx,.CC,.m,.mm")
...
call <SID>AddAlternateExtensionMapping('Presenter.js', 'View.js,.jst')
call <SID>AddAlternateExtensionMapping('View.js', 'Presenter.js,.jst')
" Setup default search path, unless the user has specified
" a path in their [._]vimrc.
if (!exists('g:alternateSearchPath'))
  let g:alternateSearchPath = 'sfr:../source,sfr:../src,sfr:../include,sfr:../inc:./tpl:./jst'
endif
...
function! DetermineExtension(path)
  ...
  for extension in keys(g:alternateExtensionsDict)
    let extLen = strlen(extension)
    if (path[strlen(path)-extLen : strlen(path)]==extension)
        return extension
    endif
  endfor
  return ""
